...
- [x] Transações recorrentes: aba "Recorrente" dentro da tela de [transações](../src/app/dashboard/transactions/transaction-form-tabs.tsx) (junto com o lançamento avulso, não em tela separada) para cadastrar uma regra (categoria, descrição, valor, dia do mês) que é lançada automaticamente como transação normal todo mês a partir da data de início, ajustando pro último dia em meses mais curtos; dá pra pausar, editar ou excluir a regra sem afetar as transações já geradas — geração é feita sob demanda (idempotente, via constraint única) ao abrir dashboard/transações/orçamento, em [recurring-transactions.ts](../src/lib/recurring-transactions.ts)/[recurring-transactions.test.ts](../src/lib/recurring-transactions.test.ts); uma transação avulsa já existente também pode ser convertida numa regra recorrente pelo dialog de edição ([transaction-row.tsx](../src/app/dashboard/transactions/transaction-row.tsx))
- [x] Contas a pagar: tela própria ([bills/page.tsx](../src/app/dashboard/bills/page.tsx)) para cadastrar uma conta com categoria, descrição, valor e vencimento, separada de recorrência (ciclo pendente → paga, `Bill.status`); ao marcar como paga, cria a `Transaction` de verdade e liga via `linkedTransactionId` — só aí ela entra nas estatísticas de gasto/orçamento; contas com "repete todo mês" geram a próxima ocorrência pendente automaticamente ao confirmar o pagamento, em [bills.ts](../src/lib/bills.ts)/[bills.test.ts](../src/lib/bills.test.ts)
